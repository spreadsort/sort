...
struct greaterthan {
  inline bool operator()(const DATA_TYPE &x, const DATA_TYPE &y) const { return x.a > y.a; }
};
...
int main(int argc, const char ** argv) {
  ...
  for(unsigned u = 0; u < loopCount; ++u) {
    ...
    start = clock();
    if(stdSort)
      //std::sort(&(array[0]), &(array[0]) + uCount, greaterthan());
      std::sort(array.begin(), array.end(), greaterthan());
    else
      //string_sort(&(array[0]), &(array[0]) + uCount, bracket(), getsize(), greaterthan());
      reverse_string_sort(array.begin(), array.end(), bracket(), getsize(), greaterthan());
    end = clock();
